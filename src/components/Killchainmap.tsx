...
export default function KillChainMap({ alerts }: KillChainMapProps) {
  ...
  const deepestStage = [...stageHits]
    .reverse()
    .find((stage) => stage.count > 0);
    ...
    >
      ...
      >
        <div>
          ...
          >
            Automatically maps detected alerts to the attacker lifecycle.
          </p>
        </div>
        ...
        >
          <div
            style={{
              fontSize: 22,
              fontWeight: 800,
              color: totalMapped > 0 ? "#f8fafc" : "#64748b",
            }}
          >
            {totalMapped}/{alerts.length}
          </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                borderRadius: 12,
                padding: "12px 10px",
                background: isActive ? `${stage.color}1f` : "#020617",
                border: `1px solid ${isActive ? stage.color : "#273449"}`,
                position: "relative",
              }}
            >
              {isDeepest && (
        ...
        {stageHits.map((stage) => (
          <div
            ...
            style={{
              height: 4,
              borderRadius: 999,
              background: stage.count > 0 ? stage.color : "#273449",
            }}
          />
        ))}
      </div>
      ...
      {deepestStage ? (
        ...
        >
          <strong>Deepest detected stage:</strong> Stage {deepestStage.id} —{" "}
          {deepestStage.name}.{" "}
          {deepestStage.id <= 2
            ? "This appears early-stage; monitor and block suspicious sources."
            : deepestStage.id <= 4
            ? "This suggests active intrusion behavior; isolate affected systems."
            : "This suggests advanced compromise; begin incident response immediately."}
        </div>
      ) : (
        <div
          style={{
