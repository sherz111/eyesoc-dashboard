...
const riskBadgeStyle = (risk: string): React.CSSProperties => ({
  ...
  fontWeight: 700,
  background:
    risk === "High" ? "#7f1d1d" : risk === "Medium" ? "#7c2d12" : "#14532d",
  color:
    risk === "High" ? "#fecaca" : risk === "Medium" ? "#fed7aa" : "#bbf7d0",
});
...
function getCorrelations(alerts: any[]) {
  ...
  if (events.has("Port Scan") && (events.has("Known Vulnerability") || events.has("Brute Force"))) {
    correlations.push({
      title: "Reconnaissance followed by access attempt",
      severity: "High",
      description: "Scanning activity was followed by exploitation or credential attack behavior.",
    });
  }

  if (events.has("Brute Force") && (events.has("Privilege Escalation") || events.has("Privileged Access"))) {
    correlations.push({
      title: "Possible account compromise path",
      severity: "High",
      description: "Repeated login failures combined with privileged activity may indicate credential compromise.",
    });
  }

  if (phases.has("Command & Control") && phases.has("Actions on Objectives")) {
    correlations.push({
      title: "Possible active compromise",
      severity: "Critical",
      description: "Command-and-control behavior appears alongside exfiltration or impact activity.",
    });
  }

  if (events.has("Malware Detected") && events.has("Ransomware Activity")) {
    correlations.push({
      title: "Malware escalation to impact",
      severity: "Critical",
      description: "Malware activity appears to have progressed toward ransomware or business-impacting behavior.",
    });
  }
...
export default function App() {
  ...
  const overallRisk =
    highCount > 0 ? "High" : medCount > 0 ? "Medium" : alerts.length > 0 ? "Low" : "None";

  const deepestStage =
    alerts
      .map((a: any) => a.killChainPhase)
      .filter(Boolean)
      .sort((a: string, b: string) => STAGE_ORDER.indexOf(b) - STAGE_ORDER.indexOf(a))[0] ||
    "None";

  const filtered = (
    riskFilter === "All" ? alerts : alerts.filter((a: any) => a.risk === riskFilter)
  ).sort((a: any, b: any) => {
    const timeA = extractTimestamp(a.details);
    const timeB = extractTimestamp(b.details);
    if (timeA === "Unknown time") return 1;
    if (timeB === "Unknown time") return -1;
  ...
  function handleSample() {
    ...
    setFileName("");
    setStreamIndex(STREAM_LOGS.length);
    setIpReputation([]);
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1180, margin: "0 auto" }}>
        ...
        >
          {[
            ...
            { label: "Alerts", value: alerts.length || "—" },
            {
              label: "Overall risk",
              value: overallRisk,
              color: overallRisk === "High" ? "#f87171" : "#f8fafc",
            },
            {
              label: "Deepest stage",
              value: deepestStage,
              color: deepestStage !== "None" ? "#38bdf8" : "#f8fafc",
            },
          ].map((card) => (
        ...
        <section style={{ ...cardStyle, marginBottom: 18 }}>
          <div
            style={{
              ...
              justifyContent: "space-between",
              alignItems: "center",
              marginBottom: showInput ? 14 : 0,
              gap: 12,
            }}
          >
            <div>
              <h2 style={{ fontSize: 16, margin: 0, color: "#f8fafc" }}>
                Log Input & Live Stream
              </h2>
              <p style={{ margin: "4px 0 0", color: "#64748b", fontSize: 13 }}>
                Paste logs, upload a file, or stream demo events one at a time.
              </p>
            </div>

            <button style={buttonStyle} onClick={() => setShowInput(!showInput)}>
              {showInput ? "Hide" : "Show"}
            </button>
          </div>
          ...
          {showInput && (
            <>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                <button
                  onClick={() => analyzeLogs(input)}
                  style={{ ...buttonStyle, border: "none", background: "#2563eb", color: "#fff", fontWeight: 700 }}
                >
                  Analyze logs
                </button>
                ...
                <button
                  ...
                  disabled={streamIndex >= STREAM_LOGS.length}
                  style={{ ...buttonStyle, opacity: streamIndex >= STREAM_LOGS.length ? 0.45 : 1 }}
                >
                  Stream next event
                </button>
                <button
                  onClick={checkIpReputation}
                  disabled={publicIps.length === 0 || checkingIps}
                  style={{
                    ...buttonStyle,
                    opacity: publicIps.length === 0 || checkingIps ? 0.45 : 1,
                  }}
                >
                ...
                <span style={{ alignSelf: "center", fontSize: 12, color: "#64748b" }}>
                  Public IPs found: {publicIps.length}
                </span>
              </div>
            </>
          )}
                ...
                {["All", "High", "Medium", "Low"].map((level) => (
                  <button
                    ...
                    style={{
                      padding: "5px 11px",
                      borderRadius: 999,
                      border: riskFilter === level ? "1px solid #60a5fa" : "1px solid #334155",
                      background: riskFilter === level ? "#1d4ed8" : "transparent",
                      color: riskFilter === level ? "#fff" : "#94a3b8",
                      fontSize: 12,
                      cursor: "pointer",
                    }}
                  >
                ...
                <thead>
                  <tr style={{ borderBottom: "1px solid #334155" }}>
                    {["Time", "Event", "Risk", "Kill Chain", "Details", "Action"].map((h) => (
                      <th key={h} style={{ padding: "10px", textAlign: "left", color: "#64748b", fontSize: 11, textTransform: "uppercase" }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {filtered.map((a: any, i: number) => (
                    <tr key={i} style={{ borderBottom: i < filtered.length - 1 ? "1px solid #1f2937" : "none" }}>
                      <td style={{ padding: 10, color: "#94a3b8", fontFamily: "monospace", fontSize: 11, whiteSpace: "nowrap" }}>
                        {extractTimestamp(a.details)}
                      </td>
                      ...
                      <td style={{ padding: 10, color: "#94a3b8", fontFamily: "monospace", fontSize: 11, maxWidth: 360 }}>
                        {a.details}
                      </td>
        ...
        {alerts.length > 0 && (
          <section style={{ ...cardStyle, marginBottom: 18 }}>
            <h3 style={{ fontSize: 15, margin: "0 0 4px", color: "#f8fafc" }}>
              Advanced Analysis
            </h3>
            <p style={{ margin: "0 0 8px", color: "#64748b", fontSize: 13 }}>
              Optional analyst details. Open only what you need.
            </p>

            <CollapsePanel
              title="Correlated Incident Findings"
              subtitle="Combines multiple alerts into higher-confidence incident patterns."
              count={correlations.length}
            >
              {correlations.length === 0 ? (
                <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
                  No correlated incident patterns detected yet.
                </p>
              ) : (
                <div style={{ display: "grid", gap: 10 }}>
                  {correlations.map((item, i) => (
                    <div
                      key={i}
                      style={{
                        background: item.severity === "Critical" ? "rgba(239,68,68,0.10)" : "rgba(249,115,22,0.10)",
                        border: item.severity === "Critical" ? "1px solid rgba(239,68,68,0.25)" : "1px solid rgba(249,115,22,0.25)",
                        borderRadius: 12,
                        padding: 12,
                      }}
                    >
                      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4 }}>
                        <span style={riskBadgeStyle(item.severity === "Critical" ? "High" : "Medium")}>
                          {item.severity}
                        </span>
            ...
            <CollapsePanel
              title="Live IP Reputation"
              subtitle="Checks public IPs from logs against AbuseIPDB through your local backend."
              count={ipReputation.length}
            >
              {ipReputation.length === 0 ? (
                <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
                  Click “Check IP Reputation” above to run live lookups.
                </p>
              ) : (
                <div style={{ overflowX: "auto" }}>
                          ...
                          <td style={{ padding: 10 }}>
                            <span
                              style={riskBadgeStyle(
                                Number(row.abuseConfidenceScore || 0) >= 75
                                  ? "High"
                                  : Number(row.abuseConfidenceScore || 0) >= 25
                                  ? "Medium"
                                  : "Low"
                              )}
                            >
            ...
            <CollapsePanel
              ...
              subtitle="Maps alerts to likely attacker tactics and techniques."
              count={mitreRows.length}
            >
              {mitreRows.length === 0 ? (
                <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
                  No MITRE mappings available for the current alerts.
                </p>
              ) : (
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
